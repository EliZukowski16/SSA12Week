...
public class PigLatinTranslator
{
    ...
    public String translate()
    {
        ...
        {
            translatedWords.add(
                    (s.length() < 2) ? ((s.matches("[aeiouAEIOU]")) ? (s + "way") : (s + "ay")) : (translateWord(s)));
        }
    ...
    private String translateWord(String s)
    {
        ...
        {
            String firstLetter = (s.substring(0, 1).matches("[A-Z]")) ? s.substring(0, 1).toUpperCase()
                    : s.substring(0, 1);
        ...
        {
            String firstTwoLetters = (s.length() == 2) ? s.substring(0, 2) + "ay"
                    : s.substring(0, 2).toLowerCase() + "ay";
            String thirdLetter = (s.length() > 2)
                    ? (s.matches("^[A-Z].*") ? s.substring(2, 3).toUpperCase() : s.substring(2, 3)) : "";
