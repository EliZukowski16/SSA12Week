...
{	
	...
	{	
		...
		int nextRandomMove;
		int[] xPossibleOffset = new int[] {-2,-1,1,2};
		int[] yPossibleOffset = new int[] {-2,-1,1,2};
...
//			System.out.println(nextMoveScaled.get(i));
		...
		for(int i = 0; i < nextMoveScaled.size(); i++)
		{
			...
			{
				if((nextRandomDecision < nextMoveScaled.get(i)) && (nextRandomDecision > nextMoveScaled.get(nextRandomMove)))
				{
					nextRandomMove = i;
				}
			}
...
//		System.out.println(nextRandomDecision + " , " + nextRandomMove + " , " + nextMoveScaled.get(nextRandomMove));
